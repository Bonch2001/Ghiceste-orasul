...
while running:
    if where == 0:
        ...
        text = font.render('Help', 1, (255, 255, 0))
        textRect = text.get_rect()
        textRect.center = (SCREEN_WIDTH/2, 3*buttonY/2)
        screen.blit(text, textRect)
        rules = [
            "Acesta este un joc tip GeoGuessr. Jucatorul primeste la fiecare runda cate o poza si trebuie sa identifice locatia.",
            " ",
            "Pasi:",
            "1. Jucatorul da click pe harta in locul de unde crede ca este poza. Pe harta va aparea un pin rosu.",
            "2. Daca este sigur cu alegerea facuta, jucatorul apasa butonul <confirm>.",
            "3. Jucatorul v-a fi informat despre cat de departe a fost, respectiv daca a castigat sau nu runda.",
            " ",
            "Pentru a castiga, jucatorul trebuie sa ajunga la 10 runde castigate.",
            "Jucatorul are 3 vieti, putand sa faca maxim 3 greseli. La a patra greseala pierde jocul.",
            " ",
            "Succes!"]
        for i, rule in enumerate(rules):
            rule_text = font_small.render(rule, 1, (255, 255, 255))
            screen.blit(rule_text, (buttonY + 20, buttonY + 100 + i * 40))
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN:
                mouse_position = pygame.mouse.get_pos()
                if mouse_position[0] < buttonX/4 and mouse_position[1] < buttonY/2:
        ...
        if mouse_position[0] < buttonX/4 and mouse_position[1] < buttonY/2:
            ...
            arrow = pygame.transform.scale(pygame.image.load("arrow.png"), [50, 50])
            screen.blit(arrow, (0, 0))
        credits = pygame.draw.rect(screen, (255,255,255), (buttonY, buttonY, SCREEN_WIDTH-2*buttonY, SCREEN_HEIGHT-2*buttonY), 0, 30)
        text = font.render('Credits', 1, (0, 0, 0))
        textRect = text.get_rect()
        textRect.center = (SCREEN_WIDTH/2, 3*buttonY/2)
        screen.blit(text, textRect)
        rules = [
            "Acest joc a fost realizat in cadrul Hackathonului ",
            "BEST Engineering Marathon (BESTEM) editia 2022 ",
            "de Alin Bonciu, Ionut Ionescu si Matei Chiscan, ",
            "studenti la Facultatea de Automatica si Calculatoare "] 
        for i, rule in enumerate(rules):
            rule_text = font.render(rule, 1, (0, 0, 0))
            screen.blit(rule_text, (buttonY + 20, buttonY + 100 + i * 40))
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN:
                mouse_position = pygame.mouse.get_pos()
